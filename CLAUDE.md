...
│   ├── stations/       # Cả 2 đều đợt 1: STATION_FLOW.md (status/isActive, chargers, giá, quyền, nearby, submit/admin duyệt trạm) — OWNER_ONBOARDING_FLOW.md (KYC, upload, hợp đồng, admin duyệt owner) — NEARBY_SEARCH.md (giải thích toán + SQL của findNearby, đọc trước khi sửa hàm đó) — STATION_FLOW.docx = bản BRD cho BA/PM — đọc trước khi code stations/owners/admin
│   └── CI_CD_Workflow.md
└── src/
